...
// MODIFICATION: Add TCP client headers
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <string.h>

// MODIFICATION: TCP client configuration
#define RCV_PORT 9999
...
// MODIFICATION: Add function to connect to the receiver
static void connect_to_receiver() {
    struct sockaddr_in serv_addr;
    if ((g_tcp_sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
    ...
    if (inet_pton(AF_INET, RCV_IP, &serv_addr.sin_addr) <= 0) {
    ...
    if (connect(g_tcp_sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        printf("\nConnection Failed to receiver at %s:%d. Will not send KPIs.\n", RCV_IP, RCV_PORT);
        g_tcp_sock = -1;
        return;
    }
    printf("Connected to KPI receiver. Ready to send data.\n");
}
...
// MODIFICATION: The entire sm_cb_kpm function is modified to format data into a string and send it.
static
void sm_cb_kpm(sm_ag_if_rd_t const* rd)
{
  ...
  // MODIFICATION: Buffer to hold the formatted string for TCP sending
...

  {
	...
	for (size_t i = 0; i < msg_frm_3->ue_meas_report_lst_len; i++) {
        ...
        // --- Log UE ID to local console ---
        log_ue_id_e2sm[type](ue_id_e2sm);

        // --- Format UE ID for TCP sending ---
        // This replicates the logic from the log_*_ue_id functions using snprintf
        switch(type) {
            case GNB_UE_ID_E2SM:
                if (ue_id_e2sm.gnb.gnb_cu_ue_f1ap_lst != NULL) {
                    for (size_t k = 0; k < ue_id_e2sm.gnb.gnb_cu_ue_f1ap_lst_len; k++) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset,
                                           "UE ID type = gNB-CU, gnb_cu_ue_f1ap = %u\n", ue_id_e2sm.gnb.gnb_cu_ue_f1ap_lst[k]);
                    }
                } else {
                    offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset,
                                       "UE ID type = gNB, amf_ue_ngap_id = %lu\n", ue_id_e2sm.gnb.amf_ue_ngap_id);
                }
            ...
            case GNB_DU_UE_ID_E2SM:
                ...
                if (ue_id_e2sm.gnb_du.ran_ue_id != NULL) {
                     offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset,
                                       "ran_ue_id = %lx\n", *ue_id_e2sm.gnb_du.ran_ue_id);
                }
                break;
            case GNB_CU_UP_UE_ID_E2SM:
                 offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset,
                                    "UE ID type = gNB-CU-UP, gnb_cu_cp_ue_e1ap = %u\n", ue_id_e2sm.gnb_cu_up.gnb_cu_cp_ue_e1ap);
                if (ue_id_e2sm.gnb_cu_up.ran_ue_id != NULL) {
                     offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset,
                                       "ran_ue_id = %lx\n", *ue_id_e2sm.gnb_cu_up.ran_ue_id);
                }
                break;
            default:
                 offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "Unknown UE ID type\n");
        }


        // --- Log measurements to local console ---
        log_kpm_measurements(&msg_frm_3->meas_report_per_ue[i].ind_msg_format_1);
        
        // --- Format measurements for TCP sending ---
        ...
        for (size_t j = 0; j < msg_frm_1->meas_data_lst_len; j++) {
            ...
            for (size_t z = 0; z < data_item.meas_record_len; z++) {
                meas_type_t const meas_type = msg_frm_1->meas_info_lst[z].meas_type;
                meas_record_lst_t const record_item = data_item.meas_record_lst[z];
                byte_array_t name = meas_type.name;

                // This replicates the logic from the log_*_value functions
                if (record_item.value == INTEGER_MEAS_VALUE) {
                    if (cmp_str_ba("RRU.PrbTotDl", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "RRU.PrbTotDl = %d [PRBs]\n", record_item.int_val);
                    } else if (cmp_str_ba("RRU.PrbTotUl", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "RRU.PrbTotUl = %d [PRBs]\n", record_item.int_val);
                    } else if (cmp_str_ba("DRB.PdcpSduVolumeDL", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "DRB.PdcpSduVolumeDL = %d [kb]\n", record_item.int_val);
                    } else if (cmp_str_ba("DRB.PdcpSduVolumeUL", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "DRB.PdcpSduVolumeUL = %d [kb]\n", record_item.int_val);
                    } else {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "Integer Measurement Name not yet supported\n");
                    }
                } else if (record_item.value == REAL_MEAS_VALUE) {
                    if (cmp_str_ba("DRB.RlcSduDelayDl", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "DRB.RlcSduDelayDl = %.2f [μs]\n", record_item.real_val);
                    } else if (cmp_str_ba("DRB.UEThpDl", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "DRB.UEThpDl = %.2f [kbps]\n", record_item.real_val);
                    } else if (cmp_str_ba("DRB.UEThpUl", name) == 0) {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "DRB.UEThpUl = %.2f [kbps]\n", record_item.real_val);
                    } else {
                        offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "Real Measurement Name not yet supported\n");
                    }
                }

                if (data_item.incomplete_flag && *data_item.incomplete_flag == TRUE_ENUM_VALUE)
                    offset += snprintf(send_buffer + offset, sizeof(send_buffer) - offset, "Measurement Record not reliable\n");
            }
        }
	}
	counter++;
  ...
  // MODIFICATION: Send the formatted buffer over TCP if connected
  if (g_tcp_sock != -1 && offset > 0) {
      send(g_tcp_sock, send_buffer, strlen(send_buffer), 0);
  }
}
...
int main(int argc, char* argv[])
{
  ...
  // MODIFICATION: Connect to the receiver application
  connect_to_receiver();
  ...
  // MODIFICATION: Close the TCP socket
  if (g_tcp_sock != -1) {
      close(g_tcp_sock);
  }
