// rc.c - A simple TCP client to receive KPIs from the kpimon_xapp
// Compile with: gcc rc.c -o rc_client
...
int main() {
    ...
    printf("Successfully connected to kpimon xApp server. Waiting for KPIs...\n");

    // --- Read data from server loop ---
    ssize_t bytes_read;
    while ((bytes_read = read(sock, buffer, BUFFER_SIZE - 1)) > 0) {
        buffer[bytes_read] = '\0'; // Null-terminate the received data
        printf("%s", buffer);      // Print the KPI report
        fflush(stdout);
    }
...
int main(int argc, char* argv[])
{
  ...
  sm_ans_xapp_t* hndl = calloc(nodes.len, sizeof(sm_ans_xapp_t)); //you define a handle for xapp, for the services that the xapp needs to execute
  assert(hndl != NULL);

  ////////////
  // START KPM
  ////////////
  int const KPM_ran_function = 2; //this is where our kpm handle starts

  //not each e2 node will support kpm, so we go through all e2 nodes...
  for (size_t i = 0; i < nodes.len; ++i) {
    e2_node_connected_xapp_t* n = &nodes.n[i];

    size_t const idx = find_sm_idx(n->rf, n->len_rf, eq_sm, KPM_ran_function);
    //..and we check if they have kpm sm in them(i.e do they support kpm or not)
    ...
    // e.g. OAI CU-CP
    if (n->rf[idx].defn.kpm.ric_report_style_list != NULL) {  //!=NULL means they have that kpm sm and the associated functionality then....
      // Generate KPM SUBSCRIPTION message
      kpm_sub_data_t kpm_sub = gen_kpm_subs(&n->rf[idx].defn.kpm); // ... then only u generate a sub req for a paritcular ran id "idx"

      hndl[i] = report_sm_xapp_api(&n->id, KPM_ran_function, &kpm_sub, sm_cb_kpm);  //to send a sub req, we create a report handle and then we send the sub request
      //now the sub manager will send sub req to the ran, and if the ran accepts then it starts sending data, and this particular handle will keep on listening for incoming msgs from the ran, then decodes the and gives you the kpms
      //sm_cb_kpm = service model callback function is referenced here so that the handle knows where to route the messages and which function has the capability to decode the incoming msgs
  ...
  for (size_t i = 0; i < nodes.len; ++i) {
    e2_node_connected_xapp_t* n = &nodes.n[i];
    //again we go through each node and check which of them offers ran fnctionality of RC
    ...
    if (n->rf[idx].defn.rc.ctrl != NULL) {
      //e2 node supports rc functionality
      // Generate RC CONTROL message
      rc_ctrl_req_data_t rc_ctrl = gen_rc_ctrl_msg(n->rf[idx].defn.rc.ctrl);

      control_sm_xapp_api(&n->id, RC_ran_function, &rc_ctrl); //this function is responsible for interacting with the iApps, the e42 interface, even this "report_sm_xapp_api" is responsible for same
      //here we are not defining a handle here because once we send a control msg we dont expect ran to give any reports to us, but we can configure it to do that
